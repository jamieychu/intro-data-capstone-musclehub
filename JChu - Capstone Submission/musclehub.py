...
df["ab_test_group"] = df.fitness_test_date.apply(lambda date: "B" if pd.isnull(date) else "A" )
...
c_table = [[250, 2254], [325, 2175]]
ch2, pval, dof, expected = chi2_contingency(c_table)
print pval
...
member_pivot = members_count.pivot(
columns = "is_member",
index = "ab_test_group",
values = "email").reset_index()
member_pivot["Total"] = members_pivot["Member"] + members_pivot["Not Member"]
member_pivot["Percent Purchase"] = members_pivot.Member / members_pivot.Total
member_pivot
...
from scipy.stats import chi2_contingency
c2_table = [[200, 50], [250, 75]]
chi2, pval, dof, expected = chi2_contingency(c2_table)
print pval
...
from scipy.stats import chi2_contingency
c3_table = [[200, 2304], [250, 2250]]
chi2, pval, dof, expected = chi2_contingency(c3_table)
print pval
